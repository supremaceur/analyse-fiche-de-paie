...
# ============================================================
_main_tab_analyse, _main_tab_fiscal = st.tabs([
    "\U0001F4CA  Analyse des fiches de paie",
    "\U0001F4B0  Simulateur fiscal — Imp\u00f4ts",
])

with _main_tab_analyse:
    # UPLOAD ZONE
    # ============================================================
...
with _main_tab_fiscal:
    # ============================================================
    # UPLOAD PAR MOIS — Onglet fiscal
    # ============================================================
    st.markdown(
        '<div class="section-header">'
        '<span class="icon">&#128197;</span> Chargez vos fiches de paie par mois'
        '</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="glass-card" style="border-left:3px solid #FFD166;margin-bottom:1.2rem;">'
        '<div style="font-size:0.9rem;color:#9CA3AF;line-height:1.7;">'
        '<strong style="color:#FFD166;">&#9888; Fiche de decembre obligatoire</strong><br>'
        'La fiche de <strong>decembre</strong> contient la valeur <em>NET FISCAL annuel</em> '
        '(cumul depuis janvier). Les autres mois sont optionnels — ils permettent de cumuler '
        'tickets restaurant et mutuelle sur l&apos;annee.<br>'
        '<em style="color:#6B7280;font-size:0.82rem;">'
        'Les fiches deposees ici sont independantes de l&apos;onglet Analyse.'
        '</em></div></div>',
        unsafe_allow_html=True,
    )
    ...
    _uploaded_mois = {}
    _cols_rows = [st.columns(4) for _ in range(3)]
    for _idx, (_num, _nom) in enumerate(_mois_list):
        _row = _idx // 4
        _col = _idx % 4
        with _cols_rows[_row][_col]:
            _is_dec = (_num == "12")
            _border = "2px solid #FFD166" if _is_dec else "1px solid rgba(255,255,255,0.08)"
            _bg = "rgba(255,209,102,0.07)" if _is_dec else "rgba(255,255,255,0.02)"
            _color = "#FFD166" if _is_dec else "#B8B5FF"
            _label = ("\U0001F4C5 " + _nom + " \u2605") if _is_dec else ("\U0001F4C4 " + _nom)
            st.markdown(
                f'<div style="border:{_border};border-radius:10px;padding:0.4rem 0.6rem;'
                f'background:{_bg};margin-bottom:0.3rem;text-align:center;">'
                f'<span style="font-size:0.82rem;font-weight:600;color:{_color};">{_label}</span></div>',
                unsafe_allow_html=True,
            )
            _f = st.file_uploader(
                _nom,
                type=["pdf"],
                key=f"fiscal_mois_{_num}",
                label_visibility="collapsed",
            )
            if _f is not None:
                _uploaded_mois[_num] = _f

    _nb_up = len(_uploaded_mois)
    if _nb_up > 0:
        _has_dec = "12" in _uploaded_mois
        if not _has_dec:
            st.markdown(
                '<div style="background:rgba(255,80,80,0.1);border:1px solid rgba(255,80,80,0.35);'
                'border-radius:8px;padding:0.5rem 0.8rem;font-size:0.85rem;'
                'color:#FF6B6B;margin:0.5rem 0;">'
                '&#9888; Fiche de decembre non chargee — '
                'le revenu fiscal annuel ne pourra pas etre calcule.</div>',
                unsafe_allow_html=True,
            )
        if st.button(
            f"\U0001F50D Analyser {_nb_up} fiche(s) selectionnee(s)",
            type="primary",
            use_container_width=True,
            key="btn_analyser_fiscal",
        ):
            _resultats_f = []
            _erreurs_f = []
            _pb = st.progress(0, text="Analyse des fiches fiscales...")
            _mois_sorted = sorted(_uploaded_mois.items())
            for _i, (_num_m, _file_m) in enumerate(_mois_sorted):
                _pb.progress((_i) / len(_mois_sorted), text=f"Analyse mois {_num_m}...")
                try:
                    _res = analyser_fiche(_file_m)
                    _resultats_f.append(_res)
                except Exception as _e:
                    _erreurs_f.append({"fichier": _file_m.name, "erreur": str(_e)})
            _pb.progress(1.0, text="Termine !")
            st.session_state["resultats_fiscal"] = _resultats_f
            st.session_state["erreurs_fiscal"] = _erreurs_f

        for _err in st.session_state.get("erreurs_fiscal", []):
            st.markdown(
                f'<div class="error-banner">Erreur sur <strong>{_err["fichier"]}</strong>'
                f' : {_err["erreur"]}</div>',
                unsafe_allow_html=True,
            )
    elif st.session_state.get("resultats_fiscal"):
        pass  # fiches deja analysees, rien a ré-uploader

    if st.session_state.get("resultats_fiscal"):
        _rf = st.session_state["resultats_fiscal"]
        _donnees_check = extraire_donnees_annuelles(_rf)
        _dec_present = not _donnees_check.get("decembre_manquant", True)
        _annee_check = _donnees_check.get("annee_detectee", "")
        st.markdown(
            f'<div class="success-banner">&#10003; {len(_rf)} fiche(s) analysee(s)'
            + (f' — annee {_annee_check}' if _annee_check else '')
            + ' — '
            + ('<strong>decembre &#10003;</strong> presente' if _dec_present
               else '<span style="color:#FFD166;">&#9888; decembre manquante</span>')
            + '</div>',
            unsafe_allow_html=True,
        )
    ...
    _resultats_dispo = st.session_state.get("resultats_fiscal", [])
    if _resultats_dispo:
